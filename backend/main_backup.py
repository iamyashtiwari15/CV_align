...
MAX_CHUNK_LENGTH = 1000  # You can tune this based on your embedding model limits

@retry(stop=stop_after_attempt(3), wait=wait_fixed(2))
def safe_upsert(collection, documents, metadatas, ids):
    collection.upsert(documents=documents, metadatas=metadatas, ids=ids)

def store_documents_in_vector_collection(document_chunks: List[Document], file_identifier: str) -> None:
    try:
        print("[DEBUG] Initializing vector collection...")
        ...
        for i, chunk in enumerate(document_chunks):
            # Limit length to prevent Ollama timeout/freeze
            content = chunk.page_content[:MAX_CHUNK_LENGTH]
            metadata = chunk.metadata if chunk.metadata else {}

            documents_content.append(content)
            documents_metadata.append(metadata)
            document_ids.append(f"{file_identifier}_{i}")

        print(f"[DEBUG] Prepared {len(documents_content)} chunks for upsert.")

        try:
            safe_upsert(vector_collection, documents_content, documents_metadata, document_ids)
            print(f"[DEBUG] Successfully upserted {len(document_chunks)} chunks.")
        except Exception as e:
            print(f"[WARN] Batch upsert failed: {e}. Trying fallback to single inserts.")
            for i in range(len(documents_content)):
                try:
                    vector_collection.upsert(
                        documents=[documents_content[i]],
                        metadatas=[documents_metadata[i]],
                        ids=[document_ids[i]],
                    )
                    print(f"[DEBUG] Upserted chunk {i+1}/{len(documents_content)} individually.")
                except Exception as inner_e:
                    print(f"[ERROR] Failed to insert chunk {i}: {inner_e}")

    except Exception as e:
        print(f"[ERROR] Failed to store documents: {str(e)}")
